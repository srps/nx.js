...
// Blocking-op stamp (defined in source/main.cc on device): fs.cc notes
// synchronous SD writes/commits so the wake classifier can veto their frame
// gaps. The host harness has no wake classifier; only the symbol is needed.
